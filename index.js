...
function makeSmartPerson(name) {
  const newSmartPerson = Object.create(smartPerson);
  newSmartPerson.name = name;
  return newSmartPerson;
...
function getCarInfoByIndex(inventory, index) {
  const result = inventory.find((item, index) => {
    return index === index;
  });
  return `The is a ${result.car_make} ${result.car_model}`
}
...
function getCarInfoById(inventory, index) {
  const result = inventory[index - 1];
...
function getGermanCars(inventory) {
  ...
  for(let i = 0; i < inventory.length - 1; i++) {
     ...
     if(car_make_value == 'BMW' || car_make_value == 'Audi' || car_make_value == 'Volkswagen' || car_make_value == 'Mercedes-Benz') {
